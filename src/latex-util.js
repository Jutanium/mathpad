...
export default {
  ...
  parse (latex, varObj, decimals) {
    ...
    latex = latex.replace(expr, (match, captureGroup) => {
      ...
      const evaluated = Number(math.eval(toEval).toFixed(decimals))
  ...
  wrapVariables(variable, latex) {
    ...
    latex = latex.replace(new RegExp(regex, 'g'), (match, p1) => { return p1 ? `*${p1}*` : match })
    // Turn 2*x* into 2 * *x*
  ...
  stripAsterisks(latex, variable, replacement) {
